...
Процедура ОбработатьКоманду() Экспорт
	...
	Лог.Информация(" Старт: Перенос обработок в каталог релиза");
	файловыеОперации.КопироватьСодержимоеКаталога(ПутьОбработокРепо, КаталогОбработокРелиза);
	файловыеОперации.ОбеспечитьПустойКаталог(ПутьОбработокРепо);
	Лог.Информация(" Конец: Перенос обработок в каталог релиза");			

	Если LoadDB Тогда
		Лог.Информация(" Старт: Перенос конфигурации в каталог релиза");
		_КомандыКонфигуратор.ВыгрузитьКонфигурациюВФайл(КаталогКонфигурацииРелиза + "/ГотоваяКонфигурацияВПрод.cf");

		Лог.Информация(" Конец: Перенос конфигурации в каталог релиза");

	КонецЕсли;
	
	Лог.Информация(" Старт: Перенос расширений в каталог релиза ");
	...
	Лог.Информация(" Конец: Перенос расширений в каталог релиза");	

	Если LoadDB Тогда
		_КомандыКонфигуратор.ПодключитьсяКХранилищу();

		Лог.Информация(" Старт: Обновление хранилища продуктива");
		...
		Лог.Информация(" Конец: Обновление хранилища продуктива");
	КонецЕсли;

	Лог.Информация(" Старт: Обновление хранилища расширений"); 
