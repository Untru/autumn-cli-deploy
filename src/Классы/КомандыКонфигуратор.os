...
Процедура Инициализировать(Идентификатор) Экспорт
	...
	//УправлениеИБ.ПутьКПриложению("C:\Program Files\1cv8\8.3.26.1581\bin\ibcmd.exe");
...
Процедура ЗагрузитьРасширенияИзФайлов() Экспорт
	...
	ЗагружатьAelita = Истина;
	ЗагружатьToolsWorld = Истина;
	...
	Если Нрег(_КомандыПараметры.ВернутьСвойство("MethodLoadDB")) = "ibcmd" Тогда
		...
		Если ЗагружатьToolsWorld Тогда
			ПутьРасширения = СтрШаблон("%1\src\cfe\ToolsWorld", _КомандыПараметры.ВернутьСвойство("PathRepository"));
			
			УправлениеИБ.ЗагрузитьКонфигурациюИзФайлов(ПутьРасширения, "ToolsWorld");
			Если НЕ УправлениеИБ.УспешноеВыполнениеКоманды Тогда
				Сообщить("Ошибка при выполнении команды ibcmd ЗагрузитьВсеРасширенияИзФайлов");
			КонецЕсли;
			УправлениеИБ.ОбновитьКонфигурациюБазыДанных("ToolsWorld");
		КонецЕсли;
		

		Если ЗагружатьAelita Тогда
			ПутьРасширения = СтрШаблон("%1\src\cfe\Aelita", _КомандыПараметры.ВернутьСвойство("PathRepository"));
			
			УправлениеИБ.ЗагрузитьКонфигурациюИзФайлов(ПутьРасширения, "Aelita");
			Если НЕ УправлениеИБ.УспешноеВыполнениеКоманды Тогда
				Сообщить("Ошибка при выполнении команды ibcmd ЗагрузитьВсеРасширенияИзФайлов");
			КонецЕсли;
			
			УправлениеИБ.ОбновитьКонфигурациюБазыДанных("Aelita");
			Если НЕ УправлениеИБ.УспешноеВыполнениеКоманды Тогда
				Сообщить("Ошибка при выполнении команды ibcmd ОбновитьКонфигурациюБазыДанных");
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		Если ЗагружатьToolsWorld Тогда
			Попытка
				Конфигуратор.ЗагрузитьРасширениеИзФайлов(СтрШаблон("%1\src\cfe\ToolsWorld", _КомандыПараметры.ВернутьСвойство("PathRepository")), "ToolsWorld");
				Конфигуратор.ОбновитьКонфигурациюБазыДанных( , , , "ToolsWorld");
			Исключение
				Сообщить("Не удалось загрузить ToolsWorld");
			КонецПопытки;
		КонецЕсли;
		
		Если ЗагружатьAelita Тогда
			Попытка
				Конфигуратор.ЗагрузитьРасширениеИзФайлов(СтрШаблон("%1\src\cfe\Aelita", _КомандыПараметры.ВернутьСвойство("PathRepository")), "Aelita");
				Конфигуратор.ОбновитьКонфигурациюБазыДанных( , , , "Aelita");
			Исключение
				Сообщить("Не удалось загрузить Aelita");
			КонецПопытки;
		КонецЕсли;
...
Процедура ВыгрузитьРасширениеВФайлы() Экспорт
	...
	Если ЗагружатьToolsWorld Тогда
		Попытка
			Конфигуратор.ВыгрузитьРасширениеВФайлы(СтрШаблон("%1\src\cfe\ToolsWorld", _КомандыПараметры.ВернутьСвойство("PathRepository")), "ToolsWorld");
		Исключение
			Сообщить("Не найдено расширение ToolsWorld");
		КонецПопытки;
	КонецЕсли;
	
	Если ЗагружатьAelita Тогда
		Попытка
			Конфигуратор.ВыгрузитьРасширениеВФайлы(СтрШаблон("%1\src\cfe\Aelita", _КомандыПараметры.ВернутьСвойство("PathRepository")), "Aelita");
		Исключение
			Сообщить("Не найдено расширение Aelita");
		КонецПопытки;
	КонецЕсли;
...
Процедура ОбновитьОбработкиВКонфигурации(ПутьКВнешнимФайлам = Неопределено, МассивИменФайлов = Неопределено) Экспорт
	...
	Для Каждого Файл Из Файлы Цикл
		...
		Попытка
			...
			Сообщить(СообщениеВЛогиПользователю);
		Исключение
			Сообщить(СтрШаблон("Обработка: %1, исключительная ошибка", Файл.ИмяБезРасширения));
		КонецПопытки;
...
Процедура СобратьОбработки() Экспорт
	...
	// записьТекста = Новый ЗаписьТекста(_КомандыПараметры.ВернутьСвойство("PathRepository") + "\build\ДатаЗагрузки.txt");
