...
Процедура Закоммитить() Экспорт
	...
	АвторКоммита = _КомандыПараметры.ВернутьСвойство("CommitAuthor");
...
Процедура СоздатьМерджРеквест() Экспорт
	...
	ТекстМерджРеквеста = _КомандыПараметры.ВернутьСвойство("TextMR");
	Если НЕ ЗначениеЗаполнено(ТекстМерджРеквеста) Тогда
		Возврат;
	КонецЕсли;
	URLGitLab = _КомандыПараметры.ВернутьСвойство("UrlRemoteRepository");
	...
	МассивРевьюверов.Добавить(_КомандыПараметры.ВернутьСвойство("IdUserGitlab"));
...
Процедура КлонироватьРепозиторий()
	...
	ПараметрыКомандыGit.Добавить("config --global --add safe.directory " + 
		СтрЗаменить(СтрЗаменить(_КомандыПараметры.ВернутьСвойство("PathRepository"), """", ""), "\", "/")
	);
	Гит.ВыполнитьКоманду(ПараметрыКомандыGit);
