...
&Опция(Имя = "NameBranche", Описание = "Название ветки")
Перем NameBranche;
...
Процедура ОбработатьКоманду() Экспорт
	...
	Если Не ЗначениеЗаполнено(NameBranche) Тогда
		NameBranche = "develop";
	КонецЕсли;
	...
	Попытка
		_КомандыГИТ.ПерейтиНаВетку(NameBranche);
	...
	Если NameBranche <> ИмяТекущейВетки Тогда
...
Функция НастройкиВыполнения()
	...
	НастройкиВыполнения.Вставить("NameBranche", NameBranche);
