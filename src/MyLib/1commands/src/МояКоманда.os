...
Процедура ЗадатьИмяЖурнала(ИмяЖурнала) Экспорт
	
	Лог = Логирование.ПолучитьЛог(ИмяЖурнала);	
	//Лог.УстановитьУровень(УровниЛога.Информация);
	Попытка
		ФайлЖурнала = Новый ВыводЛогаВФайл;
		ФайлЖурнала.ОткрытьФайл(СтрШаблон("E:\Users\!DEVOps\logs\%1.log", ИмяЖурнала));
...
Функция СформироватьСтрокуЗапускаПроцесса()
	...
	КонецЕсли;
	Лог.Информация("Полная строка запуска <%1>", СтрокаЗапуска);
