...
Перем Команда Экспорт;          // Команда исполнения вызова ibcmd
Перем ДатаНачалаВыполненияКоманды Экспорт; // Свойство класса даты начала выполнения команды
Перем УспешноеВыполнениеКоманды Экспорт; // Признак успешного выполнения команды
...
Процедура УстановитьПараметрыАвторизацииИБ(
    ...
    Если СравнитьВерсии(Версия, "8.3.18.1563") > 0 Тогда
        ...
        Если ЗначениеЗаполнено(ПользовательИБ) Тогда
            ПараметрыАвторизацииИБ.Добавить(СтрШаблон("--user=%1", ПользовательИБ));
        КонецЕсли;

        Если ЗначениеЗаполнено(ПарольИБ) Тогда
            ПараметрыАвторизацииИБ.Добавить(СтрШаблон("--password=%1", ПарольИБ));
        КонецЕсли;  
    КонецЕсли;
...
Функция ВыполнитьКоманду(УстановленныйКодВозврата = 0)
    ...
    ДатаНачалаВыполненияКоманды = ТекущаяУниверсальнаяДатаВМиллисекундах();
    ...
    Команда.ЗадатьИмяЖурнала("oscript.lib."+ ИдентификаторЛога);
    
    УспешноеВыполнениеКоманды = Истина;
    Команда.УстановитьОбработчикОжидания(ЭтотОбъект);

	КодВозврата = Команда.Исполнить();

    // Если КодВозврата <> 0 Тогда
    //     УспешноеВыполнениеКоманды = Ложь;
    // КонецЕсли;
...
Процедура ОбработкаОжиданияПроцесса(Процесс,
												ПрерываниеПроцесса,
    ...
    Таймаут = 300000;
    Если ПолучитьТекущуюДельтуКДатеНачала() > Таймаут Тогда
		Сообщить("Таймаут - прибить процесс");
...
Функция ПолучитьТекущуюДельтуКДатеНачала()
	Возврат ТекущаяУниверсальнаяДатаВМиллисекундах() - ДатаНачалаВыполненияКоманды;
КонецФункции
...
Процедура ПодменитьИмяЛога(ИмяЛога) Экспорт
