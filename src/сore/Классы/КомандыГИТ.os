...
&Пластилин Перем МенеджерПараметров; 
...
Процедура ПерейтиНаВеткуИзДевелоп() Экспорт
	...
	Если ВеткаОтсутствуетВЛокальномРепозитории Тогда	
		...
		Если ВеткаПрисутствуетВУдаленномРепозитории Тогда
			...
			Лог.Информация("Попал в апстрим");
			ПараметрыКомандыGit = Новый Массив;
			ПараметрыКомандыGit.Добавить(СтрШаблон("fetch origin %1:%1", ТекущаяВетка));
			Гит.ВыполнитьКоманду(ПараметрыКомандыGit);
		...
		Иначе
			...
			Лог.Информация("Не попал в апстрим");
			ПараметрыКомандыGit = Новый Массив;
			ПараметрыКомандыGit.Добавить(СтрШаблон("push -u origin %1", ТекущаяВетка));
			Гит.ВыполнитьКоманду(ПараметрыКомандыGit);
		КонецЕсли;
...
Процедура Инициализировать(СоздатьРепозиторий = Ложь) Экспорт
	...
	Гит.УстановитьРабочийКаталог(МенеджерПараметров.ВернутьСвойство("PathRepository"));
...
Процедура Закоммитить() Экспорт
	...
	ТекстКоммита = МенеджерПараметров.ВернутьСвойство("TextCommit");
	АвторКоммита = МенеджерПараметров.ВернутьСвойство("CommitAuthor");
...
Процедура СоздатьМерджРеквест() Экспорт
	...
	ТекстМерджРеквеста = МенеджерПараметров.ВернутьСвойство("TextMR");
	Если НЕ ЗначениеЗаполнено(ТекстМерджРеквеста) Тогда
		Возврат;
	КонецЕсли;
	URLGitLab = МенеджерПараметров.ВернутьСвойство("UrlRemoteRepository");
	ТокенПодключения = МенеджерПараметров.ВернутьСвойство("TokenConnectionGitLab");
	...
	МассивРевьюверов.Добавить(МенеджерПараметров.ВернутьСвойство("IdUserGitlab"));
	
	ДанныеМерджРеквеста = Новый Структура;
	ДанныеМерджРеквеста.Вставить("id", МенеджерПараметров.ВернутьСвойство("IdProject"));
	...
	URLЗапроса = СтрШаблон("%1/api/v4/projects/%2/merge_requests?private_token=%3", URLGitLab,
			МенеджерПараметров.ВернутьСвойство("IdProject"), ТокенПодключения);
	...
	Попытка
		...
		Лог.Информация(ТекстСообщенияМерджРеквест);
		Лог.Информация(СтрШаблон("Создание к Develop: %1", ОтветЗапроса.Текст()));
	Исключение
		Лог.Информация("Мерджреквест не создан. Исключительная ошибка!");
	КонецПопытки;
	...
	Если СтрНайти(Строка(ТекущаяВетка), "hotfix") Тогда
		...
		Попытка
			ТекстСообщенияМерджРеквест = СтрШаблон("Создан мердж реквест: %1", Строка(ОтветЗапроса.Json()["web_url"]));
			Лог.Информация(ТекстСообщенияМерджРеквест);
			Лог.Информация(СтрШаблон("Создание к Main: %1", ОтветЗапроса.Текст()));
		Исключение
			Лог.Информация("Мерджреквест не создан. Исключительная ошибка!");
		КонецПопытки;
	КонецЕсли;
...
Процедура КлонироватьРепозиторий()
	...
	Гит.КлонироватьРепозиторий(МенеджерПараметров.ВернутьСвойство("PathRemoteRepository"),
		МенеджерПараметров.ВернутьСвойство("PathRepository"));
	
	ПараметрыКомандыGit = Новый Массив;
	ПараметрыКомандыGit.Добавить("config user.name " + МенеджерПараметров.ВернутьСвойство("UserName"));
	Гит.ВыполнитьКоманду(ПараметрыКомандыGit);
	
	ПараметрыКомандыGit = Новый Массив;
	ПараметрыКомандыGit.Добавить("config user.email " + МенеджерПараметров.ВернутьСвойство("UserEmail"));
	Гит.ВыполнитьКоманду(ПараметрыКомандыGit);
	
	ПараметрыКомандыGit = Новый Массив;
	ПараметрыКомандыGit.Добавить("config --global --add safe.directory " + 
		СтрЗаменить(СтрЗаменить(МенеджерПараметров.ВернутьСвойство("PathRepository"), """", ""), "\", "/")
	);
	Гит.ВыполнитьКоманду(ПараметрыКомандыGit);
