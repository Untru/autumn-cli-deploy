...
Процедура Инициализировать() Экспорт
	...
	ДанныеСервера = СтрРазделить(МенеджерОпций.ВернутьСвойство("PathServer1с"), ":");
	
	Если ДанныеСервера.количество() = 2 Тогда
		ПараметрыСтрокиСоединения.Порт = ДанныеСервера[1];
		ПараметрыСтрокиСоединения.Сервер = ДанныеСервера[0];
	Иначе
...
Процедура ОбновитьБазуИзФайлов() Экспорт
	...
	Если Нрег(МенеджерОпций.ВернутьСвойство("MethodLoadDB")) = "ibcmd" Тогда
		...
		ЧислоПопыток = 3;
		Для Счетчик = 1 По ЧислоПопыток Цикл
...
Процедура ЗагрузитьРасширенияИзФайлов() Экспорт
	...
	Если МенеджерОпций.ЕстьСвойство("Extensions") Тогда
		...
		Если НЕ ЗначениеЗаполнено(Extensions) Тогда
			Возврат;
		КонецЕсли;
