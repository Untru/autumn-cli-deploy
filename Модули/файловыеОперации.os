...
//   Откуда - Строка - Путь к исходному каталогу
//   Куда - Строка - Путь к каталогу-назначению
//   НовоеИмя - Строка - Меняем ШаблонОбработки на новое имя
//
Процедура КопироватьСодержимоеКаталогаССозданием(Знач Откуда, Знач Куда, НовоеИмя) Экспорт
	
	Файлы = НайтиФайлы(Откуда, ПолучитьМаскуВсеФайлы());
	Для Каждого Файл Из Файлы Цикл
		ПутьКопирования = ОбъединитьПути(Куда, Файл.Имя);
		ИмяШаблона = "ШаблонОбработки";
		ПутьКопирования = СтрЗаменить(ПутьКопирования, ИмяШаблона, НовоеИмя);
		Если Файл.ЭтоКаталог() Тогда
			ОбеспечитьКаталог(ПутьКопирования);
			КопироватьСодержимоеКаталогаССозданием(Файл.ПолноеИмя, ПутьКопирования, НовоеИмя);
		Иначе
			КопироватьФайл(Файл.ПолноеИмя, ПутьКопирования);
		КонецЕсли;
	КонецЦикла;
